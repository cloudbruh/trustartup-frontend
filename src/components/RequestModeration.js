...
class RequestModeration extends React.Component {
    ...
    componentDidMount = () => {
        this.props.onTitleChanged('Запросить модерацию для стартапа ' + this.props.params.id);
    }
    ...
    handleSubmit = async (event) => {
        ...
        fd.append('content', this.state.text);
        fd.append('startup_id', this.props.params.id);

        let response = await fetch(config.url + '/api/business/request_startup',
            {
                method: 'POST',
                headers: {
    ...
    render() {
        return (
            <div className='mx-auto w-fit mt-10 p-5 flex flex-col justify-center'>
                <h1 className='font-bold text-xl mb-10'>Введите запрос:</h1>
                <div className='mb-7'>
                ...
                <div className=''>
                    <label htmlFor='files' className='font-bold mr-2'>Загрузка файлов:</label>
...
export default withParams(RequestModeration);